...
> **Status lives in `README.md` and `PROGRESS.md`; this is the methodology reference.** This file has been overwritten twice by accidental pastes — with the progress log on 2026-08-16 and with a README copy on 2026-08-26 — and restored from git both times (last from `b3317f3`, on 2026-09-24). If the first line of this file is not `# ADHD-YOLO — Project Guideline (v2, post-review)`, it has happened again: find the last good version with `git log -- PROJECT.md` and restore it with `git checkout <commit> -- PROJECT.md`.
...
|---|---|---|---|
| §6H | Scalogram normalization — per-frequency-row normalization fixed a visual problem but removes absolute band-power relationships, so theta/beta is not recoverable from the images by design | `fix/scalogram-normalization` | Done (2026-08-25, `3dc52e6`): three-view RGB encoding; TBR recoverable from the image, r = 1.000 against the signal on synthetic epochs vs −0.09 before |
...
| §6R | Inner validation split for the CV loop — Ultralytics selects `best.pt` by accuracy on the val fold and evaluation then scores on that same fold, so CV accuracy is optimistically biased | — | Done: three-way split in `run_cv`; measured AUC 0.669 → 0.503 once checkpoint selection moved off the evaluation fold |
...
Two open items remain: §6N, a known-cost decision, and §6M, a stated limitation rather than a fixable bug at 1.5 s epochs. §6H and §6R, the other two originally listed as open, are done.

---

## 7. Kickoff next steps (2026-08-12, historical)
